...
export default function ProductContainer({ match, history }){
    // const {
        // data: { isLoggedIn } 
    // }= useQuery(QUERY);
    const productId = 0;
    ...
    let uniqColor = [];
    if (option) {
        uniqColor = option.filter((item, i)=>{
            return (
                option.findIndex((item2, j)=> {
                    return item.colors === item2.colors;
                }) === i
            )
        })
    }
    ...
    useEffect(() => {
        const optionTemp =[];
        // 현재 color의 item을 가져와 option으로 설정
        if(color !== "") {
        ...
        if (size.length !== 0) {
            ...
            if (size !== "") {
                ...
                if (isAdded) {
                    ...
                    setCount([...count, 1]);
                    setTotalarr([...totalarr, data.seeproduct.price]);
                }
            }
        } 
        // eslint-disable-next-line react-hooks/exhaustive-deps
